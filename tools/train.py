...
def main(args):
    ...
    cfg = Config.fromfile(args.config) #args.config: gta2cs_uda_warm_fdthings_rcs_croppl_a999_daformer_mitb5_s0.py
    ...
    #定义model,用build_train_model函数，cfg是config文件参数，train_cfg可以从model下的那个文件里找到（里面还含了一层_base_包含的关系），train_cfg={}; test_cfg={}
    ...
    #这个文件主要看这个函数，以及model,datasets,cfg等的定义
    train_segmentor(
        model,
        datasets,
        cfg,
...
if __name__ == '__main__':
    main(sys.argv[1:]) #sys.argv[0]一般是被调用的脚本文件名或全路径，argv[1]之后的就是传入的参数了
