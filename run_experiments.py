...
# --------------------------------Training with Predefined Config-------------------------------
    ...
    if args.config is not None:
        cfg = Config.fromfile(args.config) #e.g exp=basic(congfigs里定义的), machine=local
        # Specify Name and Work Directory
        exp_name = f'{args.machine}-{cfg["exp"]}' #local-basic
...
# -----------------------Training with Generated Configs from experiments.py---------------------
    ...
    if args.exp is not None:
        exp_name = f'{args.machine}-exp{args.exp}' #e.g exp=7, exp_name:local-7
