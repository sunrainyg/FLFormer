...
class UDADataset(object):
    ...
    def __init__(self, source, target_day, target_night, cfg):
        self.source = source
        self.target_day = target_day
        self.target_night = target_night
        self.ignore_index = target_day.ignore_index
        self.CLASSES = target_day.CLASSES
        self.PALETTE = target_day.PALETTE
        assert target_day.ignore_index == source.ignore_index
        assert target_day.CLASSES == source.CLASSES
        assert target_day.PALETTE == source.PALETTE
    ...
    def get_rare_class_sample(self):
        ...
        if self.rcs_min_crop_ratio > 0:
            for j in range(10):
                ...
                s1 = self.source[i1]
        i2 = np.random.choice(range(len(self.target_day)))
        s2 = self.target_day[i2]

        i3 = np.random.choice(range(len(self.target_night)))
        s3 = self.target_night[i3]

        return {
            **s1, 'target_day_img_metas': s2['img_metas'],
            'target_day_img': s2['img'],
            'target_night_img_metas':s3['img_metas'],
            'target_night_img':s3['img']
        }
    ...
    def __len__(self):
        return len(self.source) * len(self.target_day) * len(self.target_night)
